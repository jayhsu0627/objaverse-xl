...
class SketchfabDownloader(ObjaverseSource):
    ...
    def _download_object(
        ...
        """Download the object for the given uid, saving it persistently if download_dir is given."""
        import urllib.request
        import os

        hf_url = f"https://huggingface.co/datasets/allenai/objaverse/resolve/main/{hf_object_path}"
        print('=========== TEST HERE =============')
        # Set permanent target path
        if download_dir is not None:
            final_path = os.path.join(download_dir, "hf-objaverse-v1", hf_object_path)
            os.makedirs(os.path.dirname(final_path), exist_ok=True)
        else:
            # If no download_dir is specified, we must still make a temp path for downloading
            temp_dir = tempfile.mkdtemp()
            final_path = os.path.join(temp_dir, hf_object_path)
            os.makedirs(os.path.dirname(final_path), exist_ok=True)

        temp_path_tmp = f"{final_path}.tmp"

        # Download from Hugging Face
        with open(temp_path_tmp, "wb") as file:
            with urllib.request.urlopen(hf_url) as response:
                file.write(response.read())
        os.rename(temp_path_tmp, final_path)

        # Verify SHA256
        sha256 = get_file_hash(final_path)
        ...
        return file_identifier, final_path
