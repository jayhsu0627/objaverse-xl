...
For example, a local user on linux may have `DISPLAY=:2.0`, or once we specific a monitor id from step 3 `sudo python3 start_x_server.py start 9` we need to forward this id to line `153` by change it into 
```
  if using_gpu:
